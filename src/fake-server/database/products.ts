...
import { IProduct, IProductAttribute, IProductStock } from '~/interfaces/product';
...
let productsDef: IProductDef[] = [];

// Function to fetch products from API
async function fetchAndStoreProducts() {
    try {
        const response = await axios.get('http://localhost:5000/api/products/front');
        productsDef = response.data.data || [];
    } catch (error) {
        productsDef = []; // Reset to an empty array on error
    }
}
...
function resolveProductAttributesDef(attributesDef: IProductAttributesDef): IProductAttribute[] {
    ...
    const keys = Object.values(attributesDef);
    keys.forEach((attributeName) => {
        const attributes = Object.values(attributeName);
        const valuesDef = attributesDef[attributeName];
        const attribute: IProductAttribute = {
            name: attributes[0],
            slug: nameToSlug(attributes[0]),
            featured: attributes[2],
            values: attributes[3],
        };

        // Check if the valuesDef is an array
        if (Array.isArray(valuesDef)) {
            if (typeof valuesDef[0] === 'boolean' && valuesDef[0]) {
                attribute.featured = true;
                valuesDef.splice(0, 1); // Remove the featured indicator
            }

            valuesDef.forEach((value) => {
                if (typeof value === 'string') {
                    ...
                    if (value.name && value.value) {
                        attribute.values.push({
                            name: value.value,
                            slug: nameToSlug(value.value),
                        });
                    }
                }
            });
        } else if (typeof valuesDef === 'object' && valuesDef !== null) {
            if (valuesDef.name && valuesDef.value) {
                attribute.values.push({
                    name: valuesDef.value,
                    slug: nameToSlug(valuesDef.value),
                });
            }
        }
...
// Converts fetched products to required structure
function makeProducts(defs: IProductDef[]): IProduct[] {
    return defs.map((def) => {
        ...
        return {
            ...
            rating: def.rating,
            reviews: def.reviews,
            availability: def.availability, // Keep this if needed separately
